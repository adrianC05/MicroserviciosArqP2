...
const cargarOrdenes = async () => {
  ...
  try {
    ...
    error.value = 'Error al cargar los ordenes'
    console.error(err)
  } finally {
    loading.value = false
  }
...
const eliminarOrden = async id => {
  if (!confirm('¿Eliminar este orden?')) return
  try {
    await apiService.eliminarOrden(id)
    await cargarOrdenes()
  } catch (err) {
    error.value = 'Error al eliminar el orden'
    console.error(err)
  }
}
...
<template>
  <BaseTableView
    ...
    empty-sub-message="Intenta con otros términos de búsqueda o agrega nuevos ordenes"
      ...
      <tr v-for="orden in items" 
        ...
        <td class="px-6 py-4 whitespace-nowrap">
          <div class="flex items-center space-x-3">
            ...
            <div>
              <p class="text-gray-900 font-medium">{{ orden.nombre }}</p>
              <p class="text-gray-500 text-sm">Orden #{{ orden.id }}</p>
            </div>
          </div>
        </td>
          ...
          <div class="flex items-center space-x-3">
            ...
            <div class="w-16 bg-gray-200 rounded-full h-2">
              <div class="h-2 rounded-full transition-all duration-300" 
                   :class="orden.cantidad > 10 ? 'bg-gradient-to-r from-emerald-400 to-green-500' : 'bg-gradient-to-r from-red-400 to-rose-500'"
                   :style="`width: ${Math.min(orden.cantidad / 20 * 100, 100)}%`">
              </div>
            </div>
          </div>
        </td>
        ...
        <td class="px-6 py-4 whitespace-nowrap">
          <div class="flex items-center space-x-2">
            <router-link :to="{ name: 'formElement', params: { id: orden.id } }" 
